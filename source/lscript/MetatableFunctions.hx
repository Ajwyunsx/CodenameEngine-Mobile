...
class MetatableFunctions {
    ...
    // Helper function to avoid C++ address-of issues with -1 literals
    private static function convertLuaParameter(state:State, index:Int, nparams:Int, i:Int):{result:Dynamic, specialIndex:Int, parentIndex:Int} {
        // Use new pointer-free conversion to avoid hxcpp generating &(literal)
        final conv = CustomConvert.fromLuaEx(-nparams + i, i == 0);
        return { result: conv.value, specialIndex: conv.specialIndex, parentIndex: conv.parentIndex };
    }
    ...
    static function metatableFunc(state:State, funcNum:Int) {
        ...
        var specialIndex:Int = -1;  // must be a mutable local so its address is valid
        var parentIndex:Int = -1;   // must be a mutable local so its address is valid

        // Use a helper function to avoid hxcpp generating &(-1) for addressOf on inlined constants
        var params:Array<Dynamic> = [];
        for (i in 0...nparams) {
            var conversionResult = convertLuaParameter(state, -nparams + i, nparams, i);
            
            // Update the main indices if they were modified
            if (conversionResult.specialIndex >= 0) specialIndex = conversionResult.specialIndex;
            if (conversionResult.parentIndex >= 0) parentIndex = conversionResult.parentIndex;
            
            params.push(conversionResult.result);
        }
        ...
        if (returned != null) {
            // Use a mutable variable to avoid passing -1 literal
            var tempIndex = -1;
            CustomConvert.toLua(returned, funcNum < 2 ? specialIndex : tempIndex);
            return 1;
        }
        return 0;
    }
