...
class CustomConvert {
	...
	/**
	 * Converts a lua variable to haxe and returns additional info without using pointers.
	 * Useful to avoid hxcpp generating address-of on literals.
	 */
	public static function fromLuaEx(stackPos:Int, ?includeIndexes:Bool = false):{ value:Dynamic, specialIndex:Int, parentIndex:Int } {
		var ret:Any = null;
		var special:Int = -1;
		var parent:Int = -1;
		final curLua = LScript.currentLua;
		final luaState = curLua.luaState;
		...
		if (ret is Dynamic && Reflect.hasField(ret, "__special_id")) {
			...
			if (includeIndexes) {
				special = specID;
				parent = Reflect.field(ret, "__parent_id");
			}
			return { value: curLua.specialVars[specID], specialIndex: special, parentIndex: parent };
		}
		return { value: ret, specialIndex: special, parentIndex: parent };
	}
